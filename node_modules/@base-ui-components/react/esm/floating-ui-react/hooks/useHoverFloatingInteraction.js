...
export function useHoverFloatingInteraction(context, parameters = {}) {
  ...
  const closeWithDelay = React.useCallback((event, runElseBranch = true) => {
    ...
    if (closeDelay && !handlerRef.current) {
      openChangeTimeout.start(closeDelay, () => onOpenChange(false, createChangeEventDetails(REASONS.triggerHover, event)));
  ...
  useIsoLayoutEffect(() => {
    ...
    if (open && handleCloseOptionsRef.current?.blockPointerEvents && isHoverOpen() && isElement(elements.domReference) && elements.floating) {
    ...
    return undefined;
  }, [enabled, open, elements.domReference, elements.floating, handleCloseOptionsRef, isHoverOpen, tree, parentId, performedPointerEventsMutationRef]);
  React.useEffect(() => {
    if (!enabled) {
      return undefined;
    }
