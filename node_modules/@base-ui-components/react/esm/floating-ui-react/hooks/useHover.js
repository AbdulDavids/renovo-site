...
export function useHover(context, props = {}) {
  ...
  React.useEffect(() => {
    ...
    function onLeave(event) {
      ...
      if (isHoverOpen()) {
        onOpenChange(false, createChangeEventDetails(REASONS.triggerHover, event, event.currentTarget ?? undefined));
      }
    }
  ...
  const closeWithDelay = React.useCallback((event, runElseBranch = true) => {
    ...
    if (closeDelay && !handlerRef.current) {
      timeout.start(closeDelay, () => onOpenChange(false, createChangeEventDetails(REASONS.triggerHover, event)));
  ...
  React.useEffect(() => {
    ...
    function onReferenceMouseEnter(event) {
      ...
      const isOverInactiveTrigger = elements.domReference && trigger && !contains(elements.domReference, trigger);
      if (openDelay) {
  ...
  const reference = React.useMemo(() => {
    ...
    return {
      ...
      onMouseMove(event) {
        ...
        const isOverInactiveTrigger = elements.domReference && !contains(elements.domReference, event.target);
        function handleMouseMove() {
          if (!blockMouseMoveRef.current && (!openRef.current || isOverInactiveTrigger)) {
            onOpenChange(true, createChangeEventDetails(REASONS.triggerHover, nativeEvent, trigger));
          }
        }
