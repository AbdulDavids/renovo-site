...
export function useDismiss(context, props = {}) {
  ...
  const closeOnPressOutside = useStableCallback((event, endedOrStartedInside = false) => {
    ...
    if (target && elements.triggers?.some(trigger => contains(trigger, target))) {
      return;
    }
  ...
  const handlePointerDown = useStableCallback(event => {
    if (getOutsidePressEvent() !== 'sloppy' || event.pointerType === 'touch' || !open || !enabled || isEventTargetWithin(event, elements.floating) || isEventTargetWithin(event, elements.domReference)) {
      return;
    }
  ...
  const handleTouchMove = useStableCallback(event => {
    if (getOutsidePressEvent() !== 'sloppy' || !touchStateRef.current || isEventTargetWithin(event, elements.floating) || isEventTargetWithin(event, elements.domReference)) {
      return;
    }
    const touch = event.touches[0];
    if (!touch) {
